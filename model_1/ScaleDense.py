...
class SE_block(nn.Module):
    def __init__(self, inchannels, reduction = 4):
        ...
        self.FC1 = nn.Linear(inchannels,inchannels//2)
        self.FC2 = nn.Linear(inchannels//2,inchannels)
        self.conv1 = nn.Conv3d(2, 8, (3, 3, 3), stride=1, padding=1, bias=False)
        self.conv2 = nn.Conv3d(8, 1, (3, 3, 3), stride=1, padding=1, bias=False)
    def forward(self,x):
        model_input = x
        ...
        #
        #
        # x_spatial = self.conv2(self.conv1(x_avg))
        # x_spatial = nn.Sigmoid()(x_spatial)
...
class dense_layer(nn.Module):
    ...
    def forward(self,x):
        # print("输入的x.shape:", x.shape)
        new_features = self.block(x)
        x = F.max_pool3d(x,2)
        x = torch.cat([x, new_features], 1)
        # print("输出的x.shape:", x.shape)
        return x
...
class ScaleDense(nn.Module):
    def __init__(self,nb_filter=8, nb_block=5, use_gender=True):
        ...
        self.gap = nn.AdaptiveAvgPool3d((1,1,1))
        self.max = nn.AdaptiveMaxPool3d((1, 1, 1))
        self.deep_fc = nn.Sequential(
            nn.Linear(last_channels * 3,32,bias=True),
            nn.ELU(),
            )
    ...
    def forward(self, x, male_input):
        # print("读取的x.shape:", x.shape)
        x = self.pre(x)
        # print("经过一层卷积后的x.shape:", x.shape)
        x = self.block(x)
        # x_max = self.max(x)
        # x_max = torch.reshape(x_max, (x_max.size(0), -1))
        x_new = torch.reshape(x, (x.size(0), x.size(1), -1))
        # print("x_new.shape:", x_new.shape)
        x_mean = torch.mean(x_new, dim=2)
        # print("x_mean.shape:", x_mean.shape)
        x_std = torch.std(x_new, dim=2)
        # print("x_std.shape:", x_std.shape)
        x = self.gap(x)
        x = torch.reshape(x,(x.size(0),-1))
        # print("x.shape:", x.shape)
        # print("x_mean[0, :2], x[0, :2]:", x_mean[0, :2], x[0, :2])
        x_feature = torch.cat([x_mean, x_std, x], dim=1)

        x = self.deep_fc(x_feature)
        if self.use_gender:
